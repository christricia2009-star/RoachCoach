...
@MainActor final class RadarOrchestrator: ObservableObject {
    ...
    func scan(latitude:Double,longitude:Double,radiusMiles:Double,backendURL:String) async {
        ...
        if let url = URL(string:backendURL.trimmingCharacters(in:.whitespacesAndNewlines).hasSuffix("/") ? backendURL : backendURL+"/") {
            ...
            let camera = CameraRadarSource(endpoint:url.appendingPathComponent("cameras/near").absoluteString); let cs = Date()
            do { let o = try await camera.collect(latitude:latitude,longitude:longitude,radiusMiles:radiusMiles); all += o; statuses.append(SourceStatus(id:camera.id,name:camera.displayName,status:"ok",detail:"\(o.count) observations",durationMS:Int(Date().timeIntervalSince(cs)*1000))) } catch { statuses.append(SourceStatus(id:camera.id,name:camera.displayName,status:"error",detail:error.localizedDescription,durationMS:Int(Date().timeIntervalSince(cs)*1000))) }
        }
